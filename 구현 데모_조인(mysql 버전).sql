...
-- 가격 변경 및 이벤트 내역 수정
UPDATE REALPROD rp
JOIN PRODUCT p ON p.PRONO = rp.REALNO
SET 
    rp.REAL_PRICE = p.PRICE,
    rp.REAL_ENO = NULL,
    rp.EVENTPERIOD = NULL
WHERE rp.EVENTPERIOD = '2020-12-09';   
...
JOIN (
    ...
    FROM SALES
    GROUP BY SALE_PRONO
    HAVING SUM(SALE_QUAN) >= ALL (
        SELECT SUM(SALE_QUAN)
        FROM SALES
        GROUP BY SALE_PRONO
    )
) max_sale ON rp.REALNO = max_sale.SALE_PRONO;                                        
...
UPDATE MONTHPROFIT SET PROFIT = (SELECT SUM(SALE_MONEY) FROM SALES)  WHERE DATE_DATA = '2020-12';
UPDATE MONTHPROFIT SET ORDERCOST = (SELECT SUM(ORD_MONEY) FROM ORD) WHERE DATE_DATA = '2020-12';
UPDATE MONTHPROFIT SET PERSON_EX = (SELECT SUM(SALARY) FROM EMPLOYEE) WHERE DATE_DATA = '2020-12';
UPDATE MONTHPROFIT SET NET = (PROFIT-ORDERCOST-PERSON_EX) WHERE DATE_DATA = '2020-12';
